...
export class InstallPackageComponent implements AfterViewInit {
    ...
    getData() {
        ...
        this.commandSrv.reload().subscribe(res => {
            this.projectList = res.result
                .filter(proj => !proj.isVirtualPropsProject)
                .map(proj => ({
                    key: proj.id.toString(),
                    value: proj.isCpm ? proj.projectName + ' (CPM)' : proj.projectName,
                }));
            ...
            this.commandSrv.getPackageSources().subscribe(sources => {
                if (Array.isArray(sources.result) && sources.result.length >= 1) {
                    ...
                    this.packageSourceList = sources.result.map(z => ({
                        key: z.id.toString(),
                        value: z.sourceName,
                    }));
                    ...
                    if (this.packageSourceList.length == 1) {
                        ...
                        this.installForm.controls['packageSourceId'].setValue(
                            this.packageSourceList[0].key
                        );
                    } else {
                ...
                if (this.projectList.length == 1) {
                    this.installForm.controls['projectIndex'].setValue(
    ...
    searchPackage(pageNumber: number, packageSourceId: number | null) {
        ...
        this.commandSrv
            .searchPackage(
                this.installForm.controls['searchValue'].value!.trim(),
                skip,
                take,
                packageSourceId
            )
            .subscribe(res => {
                res.result
                    .filter(
                        x => x.packageSourceId == packageSourceId || packageSourceId == null
                    )
                    .forEach(source => {
    ...
    install(packageName: string) {
        ...
        if (projectID) {
            this.commandSrv
                .installPackage(parseInt(projectID), packageName, selectedVersion)
                .subscribe(res => {
            ...
            this.loading.stopLoading();
            this.alert.error(
                `The target project isn't selected, select your target project from the drop-down beside the 'search' button.`
            );
        }
    }
    ...
    changeCollapseStatus(packageSourceId: number) {
        this.packageSourceCollapseStatus[packageSourceId] =
            !this.packageSourceCollapseStatus[packageSourceId];
    }
    ...
    getAuthors(pkg: any) {
        ...
        if (pkg.authors && Array.isArray(pkg.authors) && pkg.authors.length > 0) {
            ...
            pkg.authors &&
            (typeof pkg.authors === 'string' || pkg.authors instanceof String) &&
