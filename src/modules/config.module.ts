...
export function getConfiguration(): ExtensionConfiguration {
  const packageSources = vscode.workspace
    .getConfiguration('nugetpackagemanagergui')
    .get('packageSources') as PackageSource[];
  const requestTimeout = vscode.workspace
    .getConfiguration('nugetpackagemanagergui')
    .get('requestTimeout') as number;
