...
import { localSearchPackage } from './local.module';
...
export async function fetchPackageVersions(
  ...
  return getPackageVersions(
    packageName,
    packageSources.filter(x => x.sourceType.toString() != SourceType[SourceType.local]),
    nugetRequestTimeout,
    vscodeHttpConfig
  );
}
  ...
  const results = await Promise.all(
    packageSources.map(async src => {
      if (src.sourceType.toString() === SourceType[SourceType.local]) {
        return localSearchPackage(src, query, take, skip)
      }
