...
/**
 * Check if a Directory.Packages.props XML has CPM enabled.
 * CPM is considered enabled when:
 * - ManagePackageVersionsCentrally is explicitly set to true, OR
 * - ManagePackageVersionsCentrally is NOT present but the file contains PackageVersion elements
 *   (newer .NET SDKs default to true when Directory.Packages.props exists)
 * CPM is disabled only when ManagePackageVersionsCentrally is explicitly set to false.
 */
export function isCpmEnabled(propsXml: string): boolean {
    try {
        ...
        // Look for explicit ManagePackageVersionsCentrally property
            ...
            if (child.name === 'PropertyGroup' && child.type === 'element' && child.elements) {
                for (const prop of child.elements) {
                    if (prop.name === 'ManagePackageVersionsCentrally' && prop.type === 'element') {
                        ...
                        if (textNode) {
                            const val = textNode.text?.trim().toLowerCase();
                            explicitValue = val === 'true';
                        }
                    }
                }
            }
        ...
        // If not explicitly set, check if the file has PackageVersion elements
        // (newer .NET SDKs default ManagePackageVersionsCentrally to true)
        for (const child of projectElement.elements) {
            if (child.name === 'ItemGroup' && child.type === 'element' && child.elements) {
                if (child.elements.some(e => e.name === 'PackageVersion' && e.type === 'element')) {
                    return true;
                }
            }
        }
