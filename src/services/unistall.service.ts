...
export function remove(projectList: Project[], projectID: number, packageName: string, config: ExtensionConfiguration): ServiceResult {
...
export function removeAllPackage(projectList: Project[], packageName: string, config: ExtensionConfiguration) {
  let commandResultList: ServiceResult[] = [];

  projectList.forEach(project => {
    const packages = project.packages.filter(x => x.packageName == packageName);
    packages.forEach(pkg => {
      let commandResult = remove(projectList, project.id, pkg.packageName, config);
      if (commandResult.isSuccessful) {
