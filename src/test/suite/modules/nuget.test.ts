...
import {
  ...
  searchPackage,
  fetchPackageVersionsBatch,
} from '../../../modules/nuget.module';
...
suite('nuget.ts tests', () => {
  ...
  test('fetchPackageVersions test', async () => {
    const result: PackageVersion = await fetchPackageVersions(
  ...
  test('fetchPackageVersionsBatch test', async () => {
    const result: PackageVersion[] = await fetchPackageVersionsBatch(
    ...
    assert(
      result[0].versions != null &&
        result[0].packageName != undefined &&
        result[0].versions.length > 0
    );

    assert(result[1].packageName != null && result[1].packageName != undefined);
    assert(
      result[1].versions != null &&
        result[1].packageName != undefined &&
        result[1].versions.length > 0
    );
  }).timeout(configOptions.requestTimeout);
